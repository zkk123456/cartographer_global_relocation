...
NodeId PoseGraph2D::AddNode(
  ...
  //记录下当前激光点
  // match_point_cloud_ = constant_data->filtered_gravity_aligned_point_cloud;
...
void PoseGraph2D::DeleteTrajectoriesIfNeeded() {
  ...
  for (auto& it : data_.trajectories_state) 
  {
        ...
        InternalTrajectoryState::DeletionState::WAIT_FOR_DELETION) 
    {
      ...
      trimming_handle.DeleteNodes(it.first);
      LOG(WARNING) << "PoseGraph2D::DeleteTrajectoriesIfNeeded(),delete nodes ";
...
void PoseGraph2D::DeleteTrajectory(const int trajectory_id) {
  ...
  AddWorkItem([this, trajectory_id]() LOCKS_EXCLUDED(mutex_) {
    ...
    data_.trajectories_state.at(trajectory_id).deletion_state =
        InternalTrajectoryState::DeletionState::WAIT_FOR_DELETION;
    DeleteTrajectoriesIfNeeded();
    LOG(WARNING) << "PoseGraph2D::DeleteTrajectory123 ";
    return WorkItem::Result::kDoNotRunOptimization;
  });
...
// float PoseGraph2D::GetScore() const {
//   return 0;
// }
...
void PoseGraph2D::RunOptimization() {
  ...
  for (const int trajectory_id : node_data.trajectory_ids()) 
  {
    // LOG(WARNING) << "PoseGraph2D::RunOptimization(),ComputeLocalToGlobalTransform :  " << trajectory_id;
    // if(trajectory_id == 0)
    //   continue;
...
bool PoseGraph2D::TrimmingHandle::DeleteNodes(int trajectory_id) 
{
    LOG(WARNING) << "Attempting to delete nodes with trajectory_id: " << trajectory_id;
    // Set to hold nodes that need to be removed
    std::set<NodeId> nodes_to_remove;

    // First pass: collect nodes to remove
    for (const auto& node_id_data : parent_->data_.trajectory_nodes) 
    {

        // Check if the node matches the target trajectory_id
        if (node_id_data.id.trajectory_id == trajectory_id)
        {
            // Check if the node index is valid
            if (node_id_data.id.node_index >= 0 && 
                node_id_data.id.node_index < 100000) 
            {
                nodes_to_remove.insert(node_id_data.id); // Collect node for removal
            } 
            else 
            {
                LOG(WARNING) << "Invalid node index: " << node_id_data.id.node_index;
            }
        }
    }

    // Log the total number of nodes before deletion
    // Second pass: actually remove the nodes
    ...
    // Log the total number of nodes after deletion
    LOG(WARNING) << "Total trajectory nodes after deletion: " << parent_->data_.trajectory_nodes.size();
...
void PoseGraph2D::TrimmingHandle::TrimSubmap(const SubmapId& submap_id) {
  ...
  LOG(WARNING) << "TrimSubmap,delete trajectory_id: "  << submap_id.trajectory_id;
  LOG(WARNING) << "TrimSubmap,delete submap_index: "  << submap_id.submap_index;

  if(parent_->data_.submap_data.at(submap_id).state != SubmapState::kFinished)
    LOG(WARNING) << "TrimSubmap,parent_->data_.submap_data.at(submap_id).state: kNoConstraintSearch";
