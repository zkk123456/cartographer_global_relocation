...
//   float GetScore() const override;
  ...
  bool PerformGlobalLocalization(cartographer::sensor::PointCloud laser_point_cloud,float cutoff, transform::Rigid2d* best_pose_estimate, float* best_score) override;
...
//   cartographer::sensor::PointCloud match_point_cloud_;
  ...
  class TrimmingHandle : public Trimmable {
   public:
    ...
    bool IsFinished(int trajectory_id) const override
        EXCLUSIVE_LOCKS_REQUIRED(parent_->mutex_);
    bool DeleteNodes(int trajectory_id) 
        EXCLUSIVE_LOCKS_REQUIRED(parent_->mutex_);
