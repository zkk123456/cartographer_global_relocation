...
// 修改了这个flag名字，因为和node_main.cpp里面命名重合了
DEFINE_bool(collect_metrics_offline_node, false,
            "Activates the collection of runtime metrics. If activated, the "
            "metrics can be accessed via a ROS service.");
DEFINE_string(configuration_directory_offline_node, "",
              "First directory in which configuration files are searched, "
              "second is always the Cartographer installation to allow "
              "including files from there.");
DEFINE_string(
    configuration_basenames_offline_node, "",
...
DEFINE_string(urdf_filenames_offline_node, "",
              "Comma-separated list of one or more URDF files that contain "
              "static links for the sensor configuration(s).");
DEFINE_bool(use_bag_transforms_offline_node, true,
            "Whether to read, use and republish transforms from bags.");
DEFINE_string(load_state_filename_offline_node, "",
              "If non-empty, filename of a .pbstream file to load, containing "
              "a saved SLAM state.");
DEFINE_bool(load_frozen_state_offline_node, true,
            "Load the saved state as frozen (non-optimized) trajectories.");
DEFINE_string(save_state_filename_offline_node, "",
...
DEFINE_bool(keep_running_offline_node, false,
            "Keep running the offline node after all messages from the bag "
            "have been processed.");
DEFINE_double(skip_seconds_offline_node, 0,
...
void RunOfflineNode(const MapBuilderFactory& map_builder_factory,
  ...
  CHECK(!FLAGS_configuration_directory_offline_node.empty())
      << "-configuration_directory_offline_node is missing.";
  LOG(WARNING) << "FLAGS_configuration_directory_offline_node " << FLAGS_configuration_directory_offline_node;
  CHECK(!FLAGS_configuration_basenames_offline_node.empty())
      << "-configuration_basenames_offline_node is missing.";
  LOG(WARNING) << "FLAGS_configuration_basenames_offline_node " << FLAGS_configuration_basenames_offline_node;
  CHECK(!(FLAGS_bag_filenames_offline_node.empty() && FLAGS_load_state_filename_offline_node.empty()))
      << "-bag_filenames and -load_state_filename cannot both be unspecified.";
  std::regex regex(",");
  std::vector<std::string> bag_filenames;
  if (!FLAGS_bag_filenames_offline_node.empty()){
    std::regex regex(",");
    std::vector<std::string> if_bag_filenames(
      std::sregex_token_iterator(
        FLAGS_bag_filenames_offline_node.begin(), FLAGS_bag_filenames_offline_node.end(), regex, -1),
      std::sregex_token_iterator()
      );
  ...
  std::vector<std::string> configuration_basenames(
    std::sregex_token_iterator(
      FLAGS_configuration_basenames_offline_node.begin(), FLAGS_configuration_basenames_offline_node.end(), regex, -1),
    std::sregex_token_iterator()
    );

  std::vector<TrajectoryOptions> bag_trajectory_options(1);
  std::tie(node_options, bag_trajectory_options.at(0)) =
      LoadOptions(FLAGS_configuration_directory_offline_node, configuration_basenames.at(0));
  ...
  for (size_t bag_index = 1; bag_index < bag_filenames.size(); ++bag_index) {
    ...
    if (bag_index < configuration_basenames.size()) {
      std::tie(std::ignore, current_trajectory_options) = LoadOptions(
          FLAGS_configuration_directory_offline_node, configuration_basenames.at(bag_index));
    } else {
  ...
  if (!FLAGS_urdf_filenames_offline_node.empty()) {
    std::vector<std::string> urdf_filenames(
      std::sregex_token_iterator(
        FLAGS_urdf_filenames_offline_node.begin(), FLAGS_urdf_filenames_offline_node.end(), regex, -1),
      std::sregex_token_iterator()
      );
  ...
  Node node(node_options, std::move(map_builder), tf_buffer, cartographer_offline_node,
            FLAGS_collect_metrics_offline_node);
  if (!FLAGS_load_state_filename_offline_node.empty()) {
    node.LoadState(FLAGS_load_state_filename_offline_node, FLAGS_load_frozen_state_offline_node);
  }
    ...
    playable_bag_multiplexer.AddPlayableBag(PlayableBag(
        ...
        [&tf_publisher, tf_buffer, cartographer_offline_node, serializer](std::shared_ptr<rosbag2_storage::SerializedBagMessage> msg) {
          ...
          if (msg->topic_name  == kTfTopic || msg->topic_name  == kTfStaticTopic) {
            if (FLAGS_use_bag_transforms_offline_node) {
  ...
  while (playable_bag_multiplexer.IsMessageAvailable()) {
    ...
    if (msg.time_stamp <
        (begin_time.nanoseconds() + rclcpp::Duration(FLAGS_skip_seconds_offline_node, 0).nanoseconds())) {
      continue;
    }
#else
  ...
  if (rclcpp::ok() &&
      !(bag_filenames.empty() && FLAGS_save_state_filename_offline_node.empty())) {
    const std::string state_output_filename =
        FLAGS_save_state_filename_offline_node.empty()
            ? bag_filenames.front() + ".pbstream"
            : FLAGS_save_state_filename_offline_node;
    LOG(INFO) << "Writing state to '" << state_output_filename << "'...";
    node.SerializeState(state_output_filename,
                        true /* include_unfinished_submaps */);
  }
  if (FLAGS_keep_running_offline_node) {
