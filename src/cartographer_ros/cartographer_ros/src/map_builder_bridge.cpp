...
void MapBuilderBridge::RelocationService(cartographer::sensor::PointCloud laser_point_cloud,const double min_score,geometry_msgs::msg::Pose* pose)
{
  // map_builder_->pose_graph()->GetScore();
  LOG(INFO) << "MapBuilderBridge::RelocationService ... ";
  float best_score = -1;
  cartographer::transform::Rigid2d pose_estimate;
  map_builder_->pose_graph()->PerformGlobalLocalization(laser_point_cloud,min_score,&pose_estimate,&best_score);
  
  // 设置头部信息
  pose->position.x = pose_estimate.translation().x();
  pose->position.y = pose_estimate.translation().y();
  pose->position.z = 0.0; // Rigid2d 是 2D 平面坐标，z 设置为 0

  LOG(INFO) << "pose->position.x : '" << pose->position.x << "',pose->position.y : '" << pose->position.y << "'...";

    // 设置方向 (Orientation)
  double theta = pose_estimate.rotation().angle(); // 获取 2D 旋转角度（弧度）
  tf2::Quaternion quaternion;
  quaternion.setRPY(0.0, 0.0, theta); // 将 2D 平面旋转角度转换为四元数
  LOG(INFO) << "pose_estimate.rotation().angle() : '" << theta  << "'...";
